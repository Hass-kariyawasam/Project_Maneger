...
export default function App() {
  ...
  const [newQuiz, setNewQuiz] = useState({q:"", opt1:"", opt2:"", opt3:"", opt4:"", ans:0});
  ...
  async function addNewQuiz() {
    if (!newQuiz.q || !newQuiz.opt1 || !newQuiz.opt2 || !newQuiz.opt3 || !newQuiz.opt4) {
        showToast("Please fill all quiz fields!"); return;
    }
    await addDoc(collection(db, QUIZZES_COL), {
        q: newQuiz.q,
        opts: [newQuiz.opt1, newQuiz.opt2, newQuiz.opt3, newQuiz.opt4],
        ans: Number(newQuiz.ans),
        addedBy: profile.username,
        ts: serverTimestamp()
    });
    setNewQuiz({q:"", opt1:"", opt2:"", opt3:"", opt4:"", ans:0});
    setShowAddQuiz(false);
    showToast("Global Quiz Question Added!");
  }

  function startQuiz() {
    const allQ = dbQuizzes.length > 0 ? dbQuizzes : QUIZ_QUESTIONS; // Fallback to defaults if db empty
    const q = [...allQ].sort(() => Math.random() - 0.5);
    ...
    <div style={{background:"#030503",minHeight:"100vh",fontFamily:"'Share Tech Mono',monospace",color:"#00ff88",display:"flex",flexDirection:"column"}}>
      ...
      <div style={{background:"#0a0f0a",borderBottom:"1px solid #00ff8833",padding:"16px 20px",display:"flex",alignItems:"center",justifyContent:"space-between",position:"sticky",top:0,zIndex:200,flexShrink:0,boxShadow:"0 4px 20px rgba(0,255,136,0.05)"}}>
        <div>
          ...
          <div style={{fontSize:9,color:"#00ff8888",marginTop:4,letterSpacing:1}}>PROJECT DURATION: 13 WEEKS</div>
        </div>
                  ...
                  <div style={{display:"flex",alignItems:"center",gap:16}}>
                    ...
                    <div style={{flex:1,minWidth:0}}>
                      ...
                      <div style={{display:"flex", flexWrap:"wrap", gap:12, marginTop:10, background:"#000", padding:"8px 12px", borderRadius:8, border:"1px solid #00ff8822"}}>
                        ...
                        {u.phone && <div style={{fontSize:11, color:"#fff"}}>📞 {u.phone}</div>}
        ...
        {activeTab === "quiz" && (
          <div style={{padding:"20px 16px"}}>
            <div style={{display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:20}}>
              ...
              <button onClick={()=>setShowAddQuiz(!showAddQuiz)} style={{background:"#00ff8822", border:"1px solid #00ff88", color:"#00ff88", padding:"6px 12px", borderRadius:6, fontSize:10, cursor:"pointer", fontFamily:"'Share Tech Mono',monospace"}}>
            ...
            {showAddQuiz && (
              <div style={{background:"#0a0f0a",border:"1px solid #00ff8855",borderRadius:12,padding:16,marginBottom:24, boxShadow:"0 0 15px #00ff8811"}}>
                <div style={{fontSize:11,color:"#00ff85aa",letterSpacing:2,marginBottom:12}}>CONTRIBUTE TO GLOBAL QUIZ</div>
                <div style={{display:"flex", flexDirection:"column", gap:10}}>
                  <GInput value={newQuiz.q} onChange={e=>setNewQuiz({...newQuiz, q:e.target.value})} placeholder="Enter your question..." />
                  <div style={{display:"flex", gap:10}}>
                    <GInput value={newQuiz.opt1} onChange={e=>setNewQuiz({...newQuiz, opt1:e.target.value})} placeholder="Option 1" />
                    <GInput value={newQuiz.opt2} onChange={e=>setNewQuiz({...newQuiz, opt2:e.target.value})} placeholder="Option 2" />
                  </div>
                  <div style={{display:"flex", gap:10}}>
                    <GInput value={newQuiz.opt3} onChange={e=>setNewQuiz({...newQuiz, opt3:e.target.value})} placeholder="Option 3" />
                    <GInput value={newQuiz.opt4} onChange={e=>setNewQuiz({...newQuiz, opt4:e.target.value})} placeholder="Option 4" />
                  </div>
                  <div style={{background:"#000", padding:"10px", borderRadius:8, border:"1px solid #00ff8833", marginTop:6}}>
                    <div style={{fontSize:11,color:"#00ff85aa",marginBottom:6}}>CORRECT ANSWER</div>
                    <select value={newQuiz.ans} onChange={e=>setNewQuiz({...newQuiz, ans:e.target.value})} style={{width:"100%",background:"#0a0f0a",border:"1px solid #00ff8844",borderRadius:8,padding:"10px",color:"#00ff88",fontFamily:"'Share Tech Mono',monospace",outline:"none"}}>
                  ...
                  <Btn onClick={addNewQuiz}>UPLOAD QUESTION</Btn>
                </div>
              </div>
            )}
            ...
            {!quizActive&&!quizDone&&(
              <div style={{textAlign:"center",padding:"50px 20px", background:"#0a0f0a", border:"1px solid #00ff8833", borderRadius:16}}>
                ...
                <div style={{fontFamily:"'Orbitron',monospace",fontSize:22,color:"#ffffff",marginBottom:10}}>DBMS KNOWLEDGE TEST</div>
                <div style={{fontSize:12,color:"#00ff8566",marginBottom:20}}>Test your skills against the global database.</div>
            ...
            {quizActive&&!quizDone&&shuffled[quizQ]&&(
              <div>
                ...
                <div style={{background:"#0a0f0a",border:"1px solid #00ff8855",borderRadius:12,padding:"20px",marginBottom:20, boxShadow:"0 0 20px #00ff8811"}}>
                  ...
                  <div style={{fontSize:10,color:"#00ff8555",marginTop:10}}>Added by: {shuffled[quizQ].addedBy || "System"}</div>
                </div>
