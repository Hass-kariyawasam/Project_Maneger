...
const GInput = ({value,onChange,placeholder,type="text",onKeyDown, disabled=false}) => (
  <input type={type} value={value} onChange={onChange} onKeyDown={onKeyDown} placeholder={placeholder} disabled={disabled}
    style={{width:"100%",background: disabled ? "#000" : "#0a0f0a",border:"1px solid #00ff8844",borderRadius:8,padding:"14px 16px",color: disabled ? "#00ff8566" : "#00ff88",fontSize:15,fontFamily:"'Share Tech Mono',monospace",outline:"none",transition:"all 0.3s", opacity: disabled ? 0.7 : 1}}
    onFocus={e=>{if(!disabled){e.target.style.borderColor="#00ff88"; e.target.style.boxShadow="0 0 12px #00ff8844"}}}
    onBlur={e=>{if(!disabled){e.target.style.borderColor="#00ff8844"; e.target.style.boxShadow="none"}}} />
);
...
export default function App() {
  ...
  // Custom Quiz Form (Multiple Questions)
    ...
    const unsub = onAuthStateChanged(auth, async (user) => {
      if (user) {
        ...
        if (snap.exists()) {
          ...
          setAuthUser(user);
          // If they just signed in via Google, prompt onboarding
  ...
  async function doGoogleLogin() {
    ...
    try {
      ...
      // onAuthStateChanged will handle the redirection/onboarding screen
    } catch(e) {
  ...
  async function doRegister(isOnboarding = false) {
    ...
    try {
      ...
      if (!isOnboarding) {
        ...
        // Google Onboarding (User already exists in Auth)
  ...
  async function doLogin() {
    ...
    try {
      ...
      // If it's not an email, assume it's a username and try to find the email
  ...
  if (!authReady) return (
    <div style={{background:"#030503",minHeight:"100vh",display:"flex",alignItems:"center",justifyContent:"center",fontFamily:"'Share Tech Mono',monospace",color:"#00ff88"}}>
      <style>{`
        @keyframes text-glow-sweep { 0% { text-shadow: 0 0 5px #00ff88, 0 0 10px #00ff88; } 50% { text-shadow: 0 0 15px #00ffcc, 0 0 30px #00ffcc, 0 0 40px #00ff88; } 100% { text-shadow: 0 0 5px #00ff88, 0 0 10px #00ff88; } }
  ...
  if (screen === "google_onboard") return (
    <div style={{background:"#030503",minHeight:"100vh",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",fontFamily:"'Share Tech Mono',monospace",padding:20}}>
      ...
      `}</style>
       <div style={{width:"100%",maxWidth:450, background:"#0a0f0a",border:"1px solid #00ff8833",borderRadius:16,padding:30,boxShadow:"0 10px 30px rgba(0,255,136,0.1)"}}>
          <div style={{textAlign:"center", marginBottom: 20}}>
            {regPhoto ? <img src={regPhoto} alt="profile" style={{width:60,height:60,borderRadius:30, border:"2px solid #00ff88", marginBottom:10}}/> : <div style={{fontSize:40}}>👤</div>}
            <div style={{fontSize:18, color:"#00ff88", letterSpacing:1}}>COMPLETE YOUR PROFILE</div>
            <div style={{fontSize:12, color:"#00ff8566", marginTop:4}}>We need a few more details to set up your workspace.</div>
          </div>
          ...
          <div style={{display:"flex",flexDirection:"column",gap:16}}>
              <div style={{display:"flex",gap:16}}>
                ...
                <div style={{flex:1}}>
                  <div style={{fontSize:11,color:"#00ff85aa",marginBottom:6,letterSpacing:1}}>PHONE NO *</div>
  ...
  if (screen === "login" || screen === "register") return (
    <div style={{background:"#030503",minHeight:"100vh",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",fontFamily:"'Share Tech Mono',monospace",padding:20}}>
      ...
      <div style={{width:"100%",maxWidth:450}}>
        ...
        <div style={{textAlign:"center",marginBottom:20}}>
          <div style={{display:"inline-flex",alignItems:"center",justifyContent:"center",width:70,height:70,border:"3px solid #00ff88",borderRadius:16,marginBottom:16,background:"#0a0f0a",fontSize:35,color:"#00ff88",boxShadow:"0 0 20px #00ff8866"}}>⬡</div>
          ...
          <div style={{fontSize:12,color:"#00ff88",marginTop:12,letterSpacing:2}} className="glow-text-smooth">
            DEVELOPED BY HASSKARIYAWASAM
          </div>
        </div>
        ...
        <div style={{background:"#0a0f0a",border:"1px solid #00ff8833",borderRadius:16,padding:30,boxShadow:"0 10px 30px rgba(0,255,136,0.1)"}}>
          <div style={{display:"flex",background:"#000000",borderRadius:10,padding:6,marginBottom:20,border:"1px solid #00ff8822"}}>
          ...
          <div style={{marginBottom: 20}}>
             ...
             <div style={{textAlign:"center", margin:"16px 0", color:"#00ff8555", fontSize:12, letterSpacing:1}}>OR WITH EMAIL</div>
          </div>

          {screen === "login" ? (
            <div style={{display:"flex",flexDirection:"column",gap:16}}>
              <div>
                <div style={{fontSize:12,color:"#00ff85aa",marginBottom:8,letterSpacing:2}}>USERNAME OR EMAIL</div>
  ...
  const tabs = [
    ...
    {id:"team", label:"TEAM & PROFILE", icon:"◉"},
    ...
    <div style={{background:"#030503",minHeight:"100vh",fontFamily:"'Share Tech Mono',monospace",color:"#00ff88",display:"flex",flexDirection:"column"}}>
      ...
      <div style={{background:"#0a0f0a",borderBottom:"1px solid #00ff8833",padding:"16px 20px",display:"flex",alignItems:"center",justifyContent:"space-between",position:"sticky",top:0,zIndex:200,flexShrink:0,boxShadow:"0 4px 20px rgba(0,255,136,0.05)"}}>
        <div>
          ...
          <div style={{fontSize:9,color:"#00ff8888",marginTop:4,letterSpacing:1}}>DBMS PRACTICUM 2026 // UNIVERSITY OF RUHUNA</div>
        </div>
        <div style={{display:"flex",alignItems:"center",gap:16}}>
          <div style={{textAlign:"right"}}>
            <div style={{fontSize:14,color:profile.color,fontWeight:"bold"}}>{profile.username}</div>
            <div style={{fontSize:10,color:"#00ff8566",marginTop:2}}>LOAD: {pct}%</div>
          </div>
          
          <div onClick={() => setActiveTab("team")} style={{cursor: "pointer"}}>
            {profile.photoURL ? (
        ...
        {activeTab === "team" && (
          <div style={{padding:"20px 16px"}}>
            ...
            <div style={{marginBottom: 30}}>
              <div style={{fontSize:12,color:"#00ccff",letterSpacing:3,marginBottom:16, fontWeight:"bold", borderBottom: "1px solid #00ccff33", paddingBottom: 8}}>MY PROFILE</div>
              
              <div style={{background:"#0a0f0a",border:`1px solid ${profile.color}44`,borderRadius:12,padding:"20px", position:"relative", overflow:"hidden", boxShadow: `0 0 15px ${profile.color}22`}}>
                <div style={{display:"flex", gap:20, alignItems:"flex-start"}}>
                   {profile.photoURL ? (
                      <img src={profile.photoURL} alt="p" style={{width:70, height:70, borderRadius:16, border:`2px solid ${profile.color}`}} />
                    ) : (
                      <div style={{width:70,height:70,borderRadius:16,background:profile.color+"22",border:`2px solid ${profile.color}`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:30,color:profile.color,flexShrink:0}}>
                        {profile.username[0].toUpperCase()}
                      </div>
                    )}
                    
                    <div style={{flex: 1}}>
                      <div style={{fontSize:20,color:profile.color,letterSpacing:1, fontWeight:"bold"}}>{profile.username}</div>
                      <div style={{fontSize:12,color:"#00ff8588", marginTop:4}}>{profile.role}</div>
                      <div style={{fontSize:12,color:"#00ff8555", marginTop:4}}>Email: {profile.email}</div>
                    </div>

                    <button onClick={openEditProfile} style={{background:"transparent", border:"1px solid #00ccff55", color:"#00ccff", padding:"8px 16px", borderRadius:8, cursor:"pointer", fontFamily:"'Share Tech Mono',monospace", fontSize:12}}>EDIT</button>
                </div>

                {editProfileMode && (
                   <div style={{marginTop: 20, paddingTop: 20, borderTop: "1px solid #00ff8822"}}>
                      <div style={{display:"flex", flexDirection:"column", gap: 12}}>
                         <div>
                            <div style={{fontSize:10, color:"#00ff85aa", marginBottom:4}}>USERNAME (CANNOT CHANGE)</div>
                            <GInput value={profile.username} disabled={true} />
                         </div>
                         <div style={{display:"flex", gap:10}}>
                           <div style={{flex:1}}>
                              <div style={{fontSize:10, color:"#00ff85aa", marginBottom:4}}>PHONE NO</div>
                              <GInput value={editData.phone} onChange={e=>setEditData({...editData, phone: e.target.value})} />
                           </div>
                           <div style={{flex:1}}>
                              <div style={{fontSize:10, color:"#00ff85aa", marginBottom:4}}>TG NUMBER</div>
                         ...
                         <div style={{display:"flex", gap:10, marginTop:10}}>
                           <Btn onClick={updateProfileData} style={{flex:2}}>{formLoading ? "UPDATING..." : "SAVE CHANGES"}</Btn>
                           <button onClick={()=>setEditProfileMode(false)} style={{flex:1, background:"transparent", border:"1px solid #ff444455", color:"#ff4444", borderRadius:8, cursor:"pointer", fontFamily:"'Share Tech Mono',monospace"}}>CANCEL</button>
                         </div>
                      </div>
                   </div>
                )}
            ...
            <div style={{fontSize:12,color:"#00ff88",letterSpacing:3,marginBottom:16, fontWeight:"bold", borderBottom: "1px solid #00ff8833", paddingBottom: 8}}>TEAM OPERATIVES</div>
        ...
        {activeTab === "data" && (
          <div style={{padding:"20px 16px"}}>
            ...
            <div style={{background:"#0a0f0a",border:"1px solid #00ff8855",borderRadius:12,padding:16,marginBottom:24, boxShadow:"0 0 15px #00ff8811"}}>
              <div style={{fontSize:11,color:"#00ff85aa",letterSpacing:2,marginBottom:12}}>ADD NEW RESOURCE LINK (ANY MEMBER)</div>
            ...
            <div style={{display:"flex", flexDirection:"column", gap:12}}>
              ...
              {resources.map(r => (
                <div key={r.id} style={{background:"#0a0f0a",border:"1px solid #00ff8833",borderRadius:10,padding:"16px", display:"flex", flexDirection:"column", gap:10}}>
                  ...
                  {r.note && (
                    <div style={{background:"#000", borderLeft:"3px solid #00ff88", padding:"8px 12px", fontSize:13, color:"#00ff85aa", borderRadius:"0 8px 8px 0"}}>
                      {r.note}
                    </div>
                  )}
                </div>
              ...
              {messages.map(msg=>{
                ...
                return (
                  <div key={msg.id} style={{display:"flex",flexDirection:isMe?"row-reverse":"row",gap:10,alignItems:"flex-end"}}>
                    ...
                    {msg.photoURL ? (
                      <img src={msg.photoURL} alt="p" style={{width:32, height:32, borderRadius:8, border:`2px solid ${msg.color||"#00ff88"}55`, flexShrink:0}} />
                    ) : (
        ...
        {activeTab === "quiz" && (
          <div style={{padding:"20px 16px"}}>
            <div style={{display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:20}}>
              <div style={{fontSize:12,color:"#00ff88",letterSpacing:3, fontWeight:"bold"}}>GLOBAL QUIZ DATABASE</div>
              <button onClick={()=>setShowAddQuizSet(!showAddQuizSet)} style={{background:"#00ff8822", border:"1px solid #00ff88", color:"#00ff88", padding:"8px 14px", borderRadius:6, fontSize:12, cursor:"pointer", fontFamily:"'Share Tech Mono',monospace"}}>
            ...
            {showAddQuizSet && (
              <div style={{background:"#0a0f0a",border:"1px solid #00ff8855",borderRadius:12,padding:16,marginBottom:24, boxShadow:"0 0 15px #00ff8811"}}>
                <div style={{fontSize:11,color:"#00ff85aa",letterSpacing:2,marginBottom:12}}>CREATE A NEW QUIZ BUNDLE</div>
                <div style={{display:"flex", flexDirection:"column", gap:10}}>
                  <GInput value={newQuizTitle} onChange={e=>setNewQuizTitle(e.target.value)} placeholder="Quiz Set Title (e.g., Week 1 DBMS)" />
                  
                  <div style={{marginTop:10}}>
                     {quizQuestions.map((q, index) => (
                        <div key={index} style={{background:"#000", padding:"14px", borderRadius:8, border:"1px solid #00ff8833", marginBottom:10}}>
                           <div style={{fontSize:12, color:"#00ff88", marginBottom:8}}>Question {index + 1}</div>
                           ...
                           <div style={{marginTop:10}}>
                             <div style={{fontSize:10, color:"#00ff85aa", marginBottom:4}}>CORRECT ANSWER</div>
                             <select value={q.ans} onChange={e=>handleQuizQChange(index, 'ans', e.target.value)} style={{width:"100%",background:"#0a0f0a",border:"1px solid #00ff8844",borderRadius:8,padding:"10px",color:"#00ff88",fontFamily:"'Share Tech Mono',monospace",outline:"none"}}>
                  ...
                  <div style={{display:"flex", gap:10, marginTop:10}}>
                     ...
                     <Btn onClick={saveQuizSet} style={{flex:2}}>UPLOAD QUIZ SET</Btn>
                  </div>
                </div>
              </div>
            )}
            ...
            {!quizActive && !quizDone && !showAddQuizSet && (
               <div style={{display:"flex", flexDirection:"column", gap:12}}>
                  {dbQuizzes.length === 0 && (
                     <div style={{textAlign:"center", padding:40, background:"#0a0f0a", border:"1px solid #00ff8833", borderRadius:12}}>
                        <div style={{fontSize:14, color:"#00ff8566", marginBottom:10}}>No custom quizzes found.</div>
                        <Btn onClick={() => startQuiz(null)} style={{maxWidth:200}}>PLAY DEFAULT QUIZ</Btn>
                     </div>
                  )}
                  {dbQuizzes.map(qz => (
                     <div key={qz.id} style={{background:"#0a0f0a",border:"1px solid #00ff8855",borderRadius:12,padding:"16px", display:"flex", justifyContent:"space-between", alignItems:"center"}}>
                        <div>
                           <div style={{fontSize:16, color:"#fff", fontWeight:"bold"}}>{qz.title}</div>
                           <div style={{fontSize:11, color:"#00ff8566", marginTop:4}}>By {qz.addedBy} • {qz.questions.length} Questions</div>
                        </div>
